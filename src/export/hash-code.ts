	...
	static get_sha256_hash_code( bin: DataView ): ArrayBuffer {
		...
		for ( let bi = 0; bi < bc; ++bi ) {	// block index
			...
			if ( bi === lbi ) {	// if last block short iteration as last 2 words are data bit length
				wc = 14;
				w[ 14 ] = Math.floor( bin.byteLength / 536870912 );	// higher word of data bit length = data byte length * 8 / 2**32
				w[ 15 ] = ( bin.byteLength << 3 ) >>> 0; // lower word of data bit length
			}
			for ( let wi = 0; wi < wc; ++wi ) {	// word index to load data into worksheet
				...
				for ( let oi = 0; oi < 4; ++oi ) {	// octet index
					...
					const octet = doi < bin.byteLength ? bin.getUint8( doi ) : ( doi === bin.byteLength ? 0x80 : 0 );
					w[ wi ] |= octet << ( ( 3 - oi ) << 3 );
				}
			}
